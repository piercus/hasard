...
class ObjectHasard extends AbstractHasard {
	...
	getOpts(options) {
		const result = {...options, __hasardKeys: options.__hasardKeys, __hasardValue: options.__hasardValue};
		return result;
	}

	resolve(unresolved, runOptions) {
		// Do not resolve "value" here, it will be resolved in generate
		const overrideUnresolved = {...unresolved};
	...
	generate(context, runOptions) {
		if (context.__hasardKeys) {
			...
			const values = AbstractHasard.isHasard(context.__hasardValue) ? context.__hasardValue.run(context.__hasardKeys.length, runOptions) : Array.from({length: context.__hasardKeys.length}).fill(context.__hasardValue);
