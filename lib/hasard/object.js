...
class ObjectHasard extends AbstractHasard {
	...
	generate(ctx, runOpts) {
		if (ctx.__hasardKeys) {
			...
			const values = this.constructor.isHasard(ctx.__hasardValue) ? ctx.__hasardValue.run(ctx.__hasardKeys.length, runOpts) : new Array(ctx.__hasardKeys.length).fill(ctx.__hasardValue);

			const res = {};
			for (const [index, k] of ctx.__hasardKeys.entries()) {
				res[k] = values[index];
			}
